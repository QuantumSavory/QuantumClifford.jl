@testitem "ECC CirculantBivariateBicycle" tags=[:ecc] begin
    @static if !Sys.iswindows() && Sys.ARCH == :x86_64 && VERSION >= v"1.11"
        using QuantumClifford
        using QuantumClifford.ECC
        using QuantumClifford.ECC: bivariate_bicycle_code_k
        using Oscar
        using QECCore
        ...
        @testset "Verify number of logical qubits `k` from Table 1: wang2024coprime" begin
            ...
            test_cases = [
                (name = "BB [[54 ,  8,  6]]", a = 3,  b = 9,  ax = [0, 2, 4],  bx = [3, 1, 2],  expected_n = 54, expected_k = 8),
                (name = "BB [[98 ,  6, 12]]", a = 7,  b = 7,  ax = [3, 5, 6],  bx = [2, 3, 5],  expected_n = 98, expected_k = 6),
                (name = "BB [[126,  8, 10]]", a = 3,  b = 21, ax = [0, 2,10],  bx = [3, 1, 2],  expected_n = 126, expected_k = 8),
                (name = "BB [[150, 16,  8]]", a = 5,  b = 15, ax = [0, 6, 8],  bx = [5, 1, 4],  expected_n = 150,  expected_k = 16),
                (name = "BB [[162,  8, 14]]", a = 3,  b = 27, ax = [0,10,14],  bx = [12,1, 2],  expected_n = 162, expected_k = 8),
                (name = "BB [[180,  8, 16]]", a = 6,  b = 15, ax = [3, 1, 2],  bx  =[6, 4, 5],  expected_n = 180, expected_k = 8)
            ]
