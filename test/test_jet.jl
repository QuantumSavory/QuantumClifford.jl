...
function test_jet()
    @testset "JET checks" begin
        ...
        if n>1
            @test_broken isempty(JET.get_reports(@report_call QuantumClifford._apply!(s,c)))
            @test_broken isempty(JET.get_reports(@report_call QuantumClifford._apply!(s,sCNOT(1,2))))
            @test_broken isempty(JET.get_reports(@report_call QuantumClifford._apply!(s,sHadamard(1))))
            @test_broken isempty(JET.get_reports(@report_call QuantumClifford._apply!(s,c2,[1,2])))
        else
