@testitem "GPU Canonicalization" tags=[:cuda] begin
    ...
    GPUExt = Base.get_extension(QuantumClifford, :QuantumCliffordGPUExt)
    using Random
    ...
    if CUDA.functional()
        @testset "GPU canonicalize! correctness" begin
            for n in [10, 50, 100]
                ...
                gpu_result = GPUExt.cuda_canonicalize!(copy(gpu_stab))
        ...
        @testset "GPU canonicalize! performance" begin
            ...
            # Only algebraic independence is required for a full rank canonicalization.
            # In the limit of large n, this occurs with a probability that tends to unity.
            gpu_stab = to_gpu(cpu_stab)
            GPUExt.cuda_canonicalize!(copy(gpu_stab))
            gpu_time = @elapsed GPUExt.cuda_canonicalize!(copy(gpu_stab))
        ...
        @testset "GPU canonicalize! phases option" begin
            ...
            gpu_result_no_phases = GPUExt.cuda_canonicalize!(copy(gpu_stab); phases=false)
