...
function reinterpret(::Type{U}, p::PauliOperator) where {U<:Unsigned}
    # Extract bits and repack into requested element type
    xs = xbit(p)
    zs = zbit(p)
...
# Register methods that reference types defined later in the module. We
# avoid referring to `Tableau`, `Stabilizer` or `PauliFrame` at parse time by
# defining those methods inside an `@eval` in a helper registration function.
function _register_reinterpret_methods()
    @eval begin
...
# If the module already defined the relevant types, register the methods
# immediately; otherwise they will be registered when `reinterpret.jl` is
# included after those types are available (QuantumClifford.jl includes this
# file late in the include chain), so this is mostly defensive.
if isdefined(@__MODULE__, :Tableau) && isdefined(@__MODULE__, :Stabilizer) && isdefined(@__MODULE__, :PauliFrame)
    _register_reinterpret_methods()
end
