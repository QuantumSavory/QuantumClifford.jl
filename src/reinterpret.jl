...
function reinterpret(::Type{U}, p::PauliOperator) where {U<:Unsigned}
    ...
    nl = length(new_xz)
    if nl % 2 != 0
        throw(ArgumentError("cannot reinterpret PauliOperator: resulting backing array length $(nl) is not even (cannot split into X and Z halves)."))
    end
    half_words = nl ÷ 2
...
function reinterpret(::Type{U}, t::Tableau) where {U<:Unsigned}
    ...
    if new_rows % 2 != 0
        throw(ArgumentError("cannot reinterpret Tableau: resulting number of rows $(new_rows) is not even (cannot split into X and Z halves)."))
    end
