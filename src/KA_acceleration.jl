...
@inline function shared_memory_reduce!(
	f, buffer, value, index, ::Val{block_size}
	) where {block_size}
...
KA.@kernel inbounds = true unsafe_indices = true function kernel_mul_ordered!(
	...
	if i <= z_offset
		x_left = order_right_left ?
			const_xzs[i, j_const] : mutable_xzs[i, j_mutable]
		z_left = order_right_left ?
			const_xzs[i + z_offset, j_const] :
			mutable_xzs[i + z_offset, j_mutable]

		x_right = order_right_left ?
			mutable_xzs[i, j_mutable] : const_xzs[i, j_const]
		z_right = order_right_left ?
			mutable_xzs[i + z_offset, j_mutable] :
			const_xzs[i + z_offset, j_const]
	...
	if phases
		value = (count_ones(high) << 1) + count_ones(low)
		buffer = KA.@localmem typeof(value) (block_size,)
		...
		if index == one(index)
			if i == one(i)
				value += const_phases[j_const]
			end
			Atomix.@atomic mutable_phases[j_mutable] += value
			Atomix.@atomic mutable_phases[j_mutable] &= 0x3
		end
	end
...
function mul_ordered!(
	mutable_xzs::AbstractGPUArray{T}, mutable_phases,
	const_xzs::AbstractGPUArray{T}, const_phases;
	...
	kernel = kernel_mul_ordered!(KA.get_backend(const_xzs), default_block_size)
	kernel(
		mutable_xzs, mutable_phases, const_xzs, const_phases,
		order_right_left, phases, Val(default_block_size);
		workgroupsize = default_block_size, ndrange = (dim_x, dim_y)
		)
