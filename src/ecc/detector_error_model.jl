...
"""Build code-capacity Stim detector error model text for an ECC code.
...
"""Write Stim detector error model text to an `IO` object."""
function write_detector_error_model(io::IO, dem::AbstractString)
    write(io, dem)
    return nothing
end

"""Write Stim detector error model text to a `.dem` file."""
