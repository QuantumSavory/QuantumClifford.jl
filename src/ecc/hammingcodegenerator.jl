...
=#
module Hamming_codes
    export Hamming_generator, Hamming
    ...
    struct Hamming_generator
        block_length::Real
        message_length::Real
        Hamming_generator(block_length,message_length) = message_length > block_length ? error("message bigger than available block") : new(block_length,message_length)
    end
    
    function Hamming(c::Hamming_generator)
        #Input dependent
