...
Conjugates: The even powers `i` of the primitive element `α` can be decomposed as `i = i' * 2ˡ`, where `i'` is odd and `l > 1`.  This decomposition implies that raising `α` to an even power `αⁱ` is equivalent to `αⁱ"'" * 2ˡ`, representing a conjugate of `αⁱ`. Multiplying by a constant (such as `2ˡ`) doesn't alter the relative position of an element with respect to its conjugates within the finite field. Therefore, when constructing the minimal polynomial vector, only odd powers of primitive elements are taken into account.
