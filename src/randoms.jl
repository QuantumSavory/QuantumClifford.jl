...
function random_destabilizer(rng::AbstractRNG, n::Int; phases::Bool=true)
    ...
    phasesarray::Vector{UInt32} = if phases rand(rng, [0x0,0x2], 2n) else zeros(UInt32, 2n) end
...
struct RandDestabMemory{N,T<:Integer}
    ...
    U::Matrix{Int8}
    xzs::BitMatrix
    phasesarray::Vector{UInt32}
    phase_options::Vector{UInt32}
    arr::Vector{T}
    n::Int64
