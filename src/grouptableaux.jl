...
using LinearAlgebra
"""
Return the full stabilizer group representated by the given generating set.

Starting with a Tableau of 2^n n-qubit Pauli operators(where n is the size of 
Stabilizer s), mutiply each one by a different subset of the elements in s to 
create all 2^n unqiue elements in the group generated by s, then return the Tableau.
...
"""
For a given non-necessarily-minimal generating set,
...
identity operator if one is contained in s, and the postive identiy operator 
...
```
"""
function minimal_generating_set(s) 
...
"""
Return the full Pauli group of the given length. Phases besides + are ignored by default, 
...
product being a different tuple of n single Pauli operators representating an n-qubit Pauli operator. 
...
'''
"""
function normalizer(s::Stabilizer)
    n = nqubits(s)
    pgroup = pauligroup(n)
    ptype = typeof(s[1])
    normalizer = ptype[]
...
"""
For a given set of Paulis, return the subset of elements in s that commute with all elements in s.
...
"""
Return the subset of elements in s that have identity operators on all qubits corresponding to 
the given subset, without the entries corresponding to subset.

Iterates through each Pauli p in the group representated by the given Stabilizer s and check if 
...
'''
"""
function contract(s::Stabilizer, subset)
    result = typeof(s[1])[]
    for p in groupify(s)
        contractable = true
        for i in subset
