...
return the minimal generating set. Only real phases are interpereted.
...
Create a 4ⁿ length `Tableau`, if phases is set to true create a 4ⁿ⁺¹ length `Tableau`. Iterate 
...
"""
Return all Pauli operators with the same number of qubits as the given `Tableau` that commute with all 
operators in the `Tableau`.
